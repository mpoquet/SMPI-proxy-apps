...
#+BEGIN_SRC sh
     ...
     egrep -hor '\<MPI_[A-Za-z0-9_]*\(' ./ | sed 's/.$/,/' | sort --unique | grep -if - SIMGRID_FOLDER/src/smpi/bindings/smpi_mpi.cpp
     echo "Compile it"
     make
...
#+BEGIN_SRC shell
  ...
  echo "Run it"
  smpirun -np 2 -hostfile ./cluster_hostfile.txt -platform ./cluster_crossbar.xml ./smac2d_mpi_and_threads_with_AVX
...
#+BEGIN_SRC sh
     ...
     cp -f ../src/mpimemu/*.txt ../src/mpimemu/*.xml mpimemu/src/
