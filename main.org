...
#+BEGIN_SRC sh
     ...
     cd Benchmarks
     echo "Download the source code"
     test -e hpccg-source.tar.gz || curl -o hpccg-source.tar.gz -Lk http://mantevo.org/downloads/HPCCG-1.0.html

     echo "Unpack the code"
     mkdir HPCCG && tar -xvf HPCCG-source.tar.gz -C HPCCG --strip-components 1
